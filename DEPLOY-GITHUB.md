...
`.github/workflows/deploy-pages.yml`. После каждого push в `main`
...
синхронизируются через 2,5 секунды после изменения, с резервной проверкой раз в
30 секунд, а также при сворачивании Mini App. При
...
между устройствами при запуске через одного и того же бота. Если локальная и
облачная копии изменились независимо, игра не перезаписывает их молча: перед
запуском появляется локализованный выбор, какую копию продолжить. Всё равно не
рекомендуется одновременно продолжать одну игру на двух устройствах.
